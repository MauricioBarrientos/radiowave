...
const RadioStationCard = React.memo(({ station, isPlaying, onPlay, isDarkMode }) => {
  return (
    <div 
    ...
    >
      <div className={`station-card-content flex flex-col items-center w-full${isDarkMode ? ' dark-mode' : ''}`}
        style={{ background: isDarkMode ? 'rgba(35,39,47,0.7)' : 'rgba(255,255,255,0.7)', borderRadius: 12, padding: 8 }}>
        <div className="relative w-full flex justify-center items-center">
          ...
          />
          {isPlaying && (
            <span className="absolute top-2 right-2 animate-pulse" aria-label="Reproduciendo">
              <svg width="18" height="18" viewBox="0 0 24 24" fill="#2563eb"><circle cx="12" cy="12" r="8"/></svg>
            </span>
          )}
        </div>
        <h5 className={`card-title font-bold text-lg text-center mb-1${isDarkMode ? ' dark-mode' : ''}`}
            style={{ background: isDarkMode ? 'rgba(35,39,47,0.7)' : 'rgba(255,255,255,0.7)', borderRadius: 6, padding: '2px 8px', margin: 0 }}>
          {station.name}
        </h5>
        <p className={`card-text text-gray-500 text-center mb-2${isDarkMode ? ' dark-mode' : ''}`}
           style={{ background: isDarkMode ? 'rgba(35,39,47,0.7)' : 'rgba(255,255,255,0.7)', borderRadius: 6, padding: '2px 8px', margin: 0 }}>
          {station.frequency}
        </p>
      </div>
      <button 
        ...
        aria-label={isPlaying ? `Pausar ${station.name}` : `Escuchar ${station.name}`}
      >
